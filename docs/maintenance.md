...
首頁「焦點活動相簿」只會顯示第一個 `精選活動` 欄為 TRUE 的相簿。建議日常只勾選一個精選相簿；如同時勾選多個，前台只會取目前排序後的第一個作大型 headline feature。

- 勾選 `精選活動`：第一個精選相簿會以大型 headline feature 顯示。
- 取消勾選：顯示於「更多活動相簿」。
